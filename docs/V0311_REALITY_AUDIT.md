...
| V0311-P2-003 | PCDN/WebRTC | `disable-pcdn` stubs WebRTC and rewrites known hosts, but no real Safari evidence proves lower PCDN/MCDN usage or no playback/live side effects. | `mountPcdnDisabler()`, `test/mbga.test.ts`, front-tab resource hosts include bilivideo/upOS hosts. | "禁用 PCDN" would overclaim; this sub-rule can affect player/interactive features if Bilibili changes. | Keep the claim as "known PCDN/WebRTC suppression"; v0.3.11 policy makes this sub-rule default-off for new users until stronger A/B evidence exists. |
