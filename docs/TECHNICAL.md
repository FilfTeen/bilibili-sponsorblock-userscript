...
V0312 MBGA Safari sampling 结论为 `PASS WITH CAVEAT`，证据仍低于 HAR 级别，因此只作为 evidence caveat 约束 claim，不改变运行时默认策略。
...
V0312 Local Learning 证据只覆盖 isolated profile 下的 page-heuristic 写入、面板可见性、删除和 panel-derived refresh cleanup；raw restore、current-profile restore、organic comment scanning 和 comment feedback lock closure 未验证。
