...
| Native request guard | Return synthetic responses for a narrow topbar badge path list only when compact header is ready. | `src/platform/native-request-guard.ts`. | `test/native-request-guard.test.ts`. | Front tab showed active guard records, but no A/B. | Need prove handled paths are redundant in current Bilibili. | Endpoint semantics can change. | Keep narrow; document as partial. |
...
| MBGA telemetry suppression | Attempt to reduce known telemetry hosts. | `mountBlockTracking`, `resolveMbgaNetworkDecision`. | `test/mbga.test.ts`. | Front tab observed data.bilibili.com open record but not definitive blocked send proof. | Need on/off Network export. | Misleading if called complete. | Rework evidence before expanding. |
