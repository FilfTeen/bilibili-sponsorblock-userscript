...
`v0.3.10` 是控制台交互补丁后的发布阻塞修复：清理诊断报告页面 URL 中的 query/hash 敏感信息，并确保 tag 构建生成不带 `v` 前缀的 userscript 版本号。`v0.3.8` 仍是仓库重命名和 Tampermonkey 更新链路迁移版本。
...
- **MBGA 生态噪音压制（best-effort）**：基于少量已知规则尝试减少部分遥测/追踪噪音，对部分 PCDN / WebRTC 路径做 best-effort 压制，并补充低侵入页面小修。它不是完整隐私防护或完整 PCDN 禁用工具。
...
- [v0.3.7 / v0.3.8 / v0.3.9 / v0.3.10 分支健康记录](./docs/BRANCH_HEALTH_V037.md)：发布后 main、tag、integration 和历史分支状态。
...
- 评论过滤、评论属地、动态过滤。
- 紧凑视频顶栏和灰字搜索行为。
- MBGA 已知网络噪音压制、部分 PCDN / WebRTC 路径 best-effort 压制、URL 清理和 UI 简化。
