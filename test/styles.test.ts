...
describe("shared glass contexts", () => {
  ...
  it("aligns color cards with the panel card selection feedback", () => {
    ...
    expect(styles).toMatch(
      /\.bsb-tm-color-field:not\(\[data-hover-suppressed="true"\]\):hover \.bsb-tm-color-controls input:not\(:focus\),[\s\S]*\.bsb-tm-color-field:focus-within \.bsb-tm-color-controls input:not\(:focus\) \{[\s\S]*border-color: rgba\(var\(--bsb-brand-blue-rgb\), 0\.22\);[\s\S]*0 8px 18px rgba\(15, 23, 42, 0\.055\);/
    );
    expect(styles).toMatch(
      /\.bsb-tm-color-field\.compact:not\(\[data-hover-suppressed="true"\]\):hover \.bsb-tm-color-controls input:not\(:focus\),[\s\S]*\.bsb-tm-color-field\.compact:focus-within \.bsb-tm-color-controls input:not\(:focus\) \{[\s\S]*0 6px 14px rgba\(15, 23, 42, 0\.045\);/
    );
  });
